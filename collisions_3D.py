...
_ROBOT1_LOC = 'Robot1Base:'
_ROBOT2_LOC = 'Robot2Base:'
_ROBOT1 = 'Robot1Links:'
_ROBOT2 = 'Robot2Links:'
...
class RobotArm:
    def __init__(self, d_values, a_values, alpha_values, root=None):
        if len(d_values) != len(a_values) or len(d_values) != len(alpha_values):
            print("Not valid DH parameters")
        ...
        self.lims = np.tile(np.array([[-np.pi, np.pi]]), (len(d_values), 1))


    def fk(self, q):
        '''
        Compute forward kinematics for this robot arm given a list of joint angles q

        returns list of pts of joint ends
        '''
        pts = [np.append(np.array(self.root), 1)]
        T = np.eye(4)

        for d, theta, a, alpha in zip(self.d_values, q, self.a_values, self.alpha_values):
            ct = cos(theta)
            st = sin(theta)
            ca = cos(alpha)
            sa = sin(alpha)
        ...
        return np.array([pt[0:3] for pt in pts])
    ...
    def draw(self, q, color='b', show=False, base_color='g'):
        ...
        ax.scatter(pts[0, 0], pts[0, 1], pts[0, 2], color=base_color)
        ax.scatter(pts[1:,0], pts[1:,1], pts[1:, 2], color=color)

        for i in range(len(pts)-1):
            ax.plot([pts[i, 0], pts[i+1,0]], [pts[i,1], pts[i+1,1]], [pts[i,2], pts[i+1,2]], color=color)
