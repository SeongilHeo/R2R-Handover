...
class TwoRevoluteRobotChain:
    def __init__(self, link_lengths, root=None):
        '''
        Build a robot comprised of revolute joints with links of length provided and the
        '''
        self.robot1_link_lengths = link_lengths[0:3]
        self.robot2_link_lengths = link_lengths[3:-1]
        if root[0] is None:
            self.robot1_root = np.zeros(2)
        if root[1] is None:
            self.robot2_root = np.zeros(2)
        self.robot1_lims = np.array([[-pi, pi], [-pi, pi], [-pi, pi]])
        self.robot2_lims = np.array([[-pi, pi], [-pi, pi], [-pi, pi]])

    def fk(self, q):
        '''
        Compute forward kinematics

        returns tuples of (pts1, pts2)
        where pts1 is coordinates of robot 1 link ends, pts 2 is coordinates of robot 2 link ends
        '''
        return (self._fk(q[0:3]), self._fk(q[3:-1]))
